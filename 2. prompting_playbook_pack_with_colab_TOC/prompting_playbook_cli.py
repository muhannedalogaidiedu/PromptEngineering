...
Providers supported (choose with --provider):
  ...
  - gemini  (pip install google-generativeai; export GOOGLE_API_KEY or GEMINI_API_KEY)
...
def make_llm(provider: str, model: str, temperature: float):
    ...
    if fn is None:
        raise SystemExit(f"Unsupported provider '{provider}'. Choose from: dummy, gemini, openai, anthropic, mistral, cohere")
